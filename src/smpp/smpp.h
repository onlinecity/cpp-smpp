...
class SmppAddress {
 public:
  std::string value_;
  uint8_t ton_;  // type-of-number
  uint8_t npi_;  // numbering-plan-indicator

  SmppAddress(const std::string &value,
              const uint8_t &ton = smpp::TON_UNKNOWN,
              const uint8_t &npi = smpp::NPI_UNKNOWN) :
    value_(value),
    ton_(ton),
    npi_(npi) {
  }
